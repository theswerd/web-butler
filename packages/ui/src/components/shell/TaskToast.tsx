...
export function TaskToast({ task, onOpen, onDismiss }: TaskToastProps) {
  ...
  return (
    ...
    <div
      ...
      className="webbutler:flex webbutler:w-full webbutler:cursor-pointer webbutler:items-center webbutler:gap-2 webbutler:rounded-full webbutler:border webbutler:border-[var(--wc-border)] webbutler:bg-[var(--wc-surface)] webbutler:py-1.5 webbutler:pr-1.5 webbutler:pl-3 webbutler:backdrop-blur-2xl webbutler:backdrop-saturate-150 webbutler:transition-colors webbutler:duration-100 webbutler:hover:bg-[var(--wc-hover-1)]"
    >
      <span
        aria-hidden
