...
export type PageHighlight = {
  ...
  selector: string;
  /** 1-3 word name shown in the marker's on-page pill ("Auto-renewal",
      "Price per minute"). Falls back to "Note n" when absent. */
  label?: string;
...
/**
 * The answer that was on screen when the user sent their next message.
 * Whatever's open in the answer card is almost certainly what "this",
 * "that", or an unnamed follow-up refers to — so it rides along as turn
 * context. Only prose answers and artifacts qualify (status pills and
 * error cards aren't "answers"), and it's skipped when the message is an
 * explicit follow-up onto the same task — that agent session already
 * remembers its own answer.
 */
export type OpenAnswerContext = {
  /** The request that produced the on-screen answer. */
  prompt: string;
  /** 'answer' (in-page card) or 'artifact' (report in the side panel). */
  tier: 'answer' | 'artifact';
  /** Artifact title, when there is one. */
  title?: string;
  /** The answer's markdown, clipped by the sender. */
  text: string;
};
...
export type RunResult = {
  ...
  extensionId?: string;
  /** Artifact tier: the published report's id — the handoff card's "Open"
      targets exactly this report, not just whatever is newest. */
  reportId?: string;
 ...
 * (synced to the server on settle so history replays across sessions) and
 * the side panel renders it while the task runs.
 *
 * Beyond the prose kinds, structured entries get custom UI in the panel:
 * 'browser' rows mirror each ghost-cursor act in the user's tab as it
 * happens; 'answer' / 'report' / 'extension' / 'highlights' land when the
 * turn settles — the results, rendered as cards and chips rather than
 * text.
 */
export type TaskUpdate = {
  ...
  /** thought = agent reasoning (dim), message = reply text, tool = action,
      ...
      browser = one browser-control act in the user's tab, answer /
      report / extension / highlights = settle-time result entries. */
  kind:
    | 'thought'
    | 'message'
    | 'tool'
    ...
    | 'extension'
    | 'highlights';
  /** The line itself; for 'answer' the full reply markdown, for 'report' /
      'extension' the title, for 'highlights' the summary line. */
  text: string;
  /** kind 'browser': which verb, for iconography. */
  verb?: BrowserAction['kind'];
  /** kind 'report' | 'extension': the secondary line (description). */
  detail?: string;
  /** kind 'highlights': the page markers, rendered as clickable chips
      that focus the marker in the origin tab. */
...
export type BrowserAction = { id: string } & (
  ...
  | { kind: 'snapshot' }
  | { kind: 'read' }
  /** Capture the tab's viewport as a JPEG (returned base64; the VM-side
      CLI writes it to a file the agent opens to SEE the page). */
  | { kind: 'screenshot' }
  | { kind: 'navigate'; url: string }
  | { kind: 'back' }
  | { kind: 'click'; ref: string }
  | { kind: 'type'; ref: string; text: string; submit?: boolean }
  /** Pick a <select>'s option by visible text — native dropdown popups
      are OS chrome, so the value is set directly and change events fired. */
  | { kind: 'select'; ref: string; option: string }
  /** A key or modifier combo: "Enter", "Ctrl+A", "Shift+Tab". */
  | { kind: 'key'; key: string }
  ...
  /** Dump the tab's recent network traffic (XHR/fetch by default) captured
      ...
      filter; "all" widens past XHR/fetch to every request. `seq` instead
      fetches one captured call in full (headers + stored bodies). */
  | { kind: 'network'; filter?: string; seq?: number }
);
...
export type CursorCommand =
  ...
  /** A one-beat camera flash over the viewport: the agent just took a
      screenshot of this tab. */
  | { kind: 'flash' }
  /** A transient floating pill ("Reviewing network traffic") for agent
      activity that has no visible cursor motion. Auto-clears in the UI. */
...
export const MESSAGE = {
  ...
  /** Background → panel liveness probe: any open panel answers `true`.
      Some Chromium forks (Vivaldi) show the panel without reporting a
      SIDE_PANEL context, so the document itself is the source of truth
      for "did the panel actually open?". */
  PANEL_PING: 'web-butler/panel-ping',
  ...
  /** The "Ask Web Butler" context-menu item was clicked on a text
      selection: the tab's shell opens with the selection's element
      attached as a picked reference, prompt focused. */
  ASK_SELECTION: 'web-butler/ask-selection',
  /** Content → background: page-specific starter prompts for the empty
      prompt box, generated by the user's own AI on the server. Cached per
      page for the session; resolves to a string[] (possibly empty). */
  STARTERS_GET: 'web-butler/starters-get',
  /** Content → background: can the butler actually work right now? Checks
      the server's health endpoint and whether any provider is connected.
      Resolves to an Availability. */
  AVAILABILITY_GET: 'web-butler/availability-get',
} as const;

/** Whether the butler can do real work right now (AVAILABILITY_GET). The
    shell turns a false into a quiet notice in the prompt box instead of
    letting sends fail mysteriously. */
export type Availability = {
  /** The Web Butler server answered its health check. */
  server: boolean;
  /** At least one AI provider is signed in (meaningless when !server). */
  provider: boolean;
};
...
export type WebButlerMessage =
  ...
  | {
      ...
      /** The answer open on screen at send time — implicit context for
          "this"/"that" follow-ups that start a NEW task. */
  ...
  | { type: typeof MESSAGE.PANEL_PING }
  ...
  | { type: typeof MESSAGE.ASK_SELECTION; text: string }
  | {
      type: typeof MESSAGE.STARTERS_GET;
      url: string;
      title: string;
      /** Compact page digest (capturePageSignal). */
      signal: string;
    }
  | { type: typeof MESSAGE.AVAILABILITY_GET };
